...
def verify():
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute("SELECT order_id, seat_id, train_id FROM orders ORDER BY create_time DESC LIMIT 1")
            o = cursor.fetchone()
            if not o:
                print('未找到订单，无法验证。')
                return
            ...
            seat_id = o['seat_id']
            train_id = o['train_id']
            print(f'选定订单 {order_id} 对应 seat_id={seat_id}, train_id={train_id}，将其状态更新为已退票 (2)')
